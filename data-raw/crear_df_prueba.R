...
df_prueba <- tibble(X_factor, X_numeric, X_integer, X_logical, X_text, X_POSIXct)
map(df_prueba, class) #- Ok class gives two for X_positx


#- le voy a poner dos rows que sean NA
df_prueba <- df_prueba %>% add_row(.before = 4) %>% add_row(.before = 9)
#- pongo dos ceros
df_prueba$X_numeric[17] <- 0
df_prueba$X_numeric[19] <- 0
df_prueba$X_numeric[18]  <- NA
#- le voy a poner un NA


library(usethis)
# use_data(df_prueba, overwrite = TRUE) #- lo guarde con esta linea que usa el pkg usethis
